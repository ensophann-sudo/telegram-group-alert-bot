...
def get_today_messages():
    ...
    FROM messages WHERE date_text = ? ORDER BY created_at ASC
    """, (today,))
    rows = cur.fetchall()
    conn.close()
    return rows
...
async def sendnow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open(file_name, "rb") as f:
        await context.bot.send_document(chat_id=update.effective_chat.id, document=f, caption="Today's messages")

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "/start - Show info\n/setme - Save chat\n/status - Check status\n/sendnow - Send Excel now\n/help - Help"
    await update.message.reply_text(text)
